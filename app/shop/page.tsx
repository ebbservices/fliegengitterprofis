...
import { getAllProducts, type ProductData } from '@/lib/data/products';
import { FALLBACK_PRODUCTS } from '@/lib/data/fallbacks';

const HANDLE_TO_ROUTE: Record<string, string> = {
  'fliegengitter-nach-mass': 'fliegengitter',
  'plissee-nach-mass': 'plissee',
  'lichtschachtabdeckung-nach-mass': 'lichtschacht',
};

const FEATURES: Record<string, string[]> = {
  fliegengitter: ['Höhe & Breite frei wählbar', 'Verschiedene Farben & RAL-Optionen', 'Rahmen- & Gittertyp auswählbar'],
  plissee: ['Höhe & Breite individuell', 'Verschiedene Plissee-Typen', 'Optional RAL-Farben'],
  lichtschacht: ['Individuelle Maße', 'Verschiedene Farboptionen', 'Robust & langlebig'],
};
...
export default async function ShopPage() {
  ...
  if (products.length === 0) {
    products = Object.values(FALLBACK_PRODUCTS);
  }
          ...
          <div className="container mx-auto">
            <div className="grid md:grid-cols-3 gap-8">
              {products.map((product: ProductData) => {
                const route = HANDLE_TO_ROUTE[product.handle] ?? product.handle;
                const startPrice = (product.metadata.pricing.min_price_cents / 100).toFixed(2).replace('.', ',');
                const features = FEATURES[route] ?? [];
